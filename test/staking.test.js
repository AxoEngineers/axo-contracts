...
describe("AxolittlesStaking", () => {
  ...
  beforeEach(async () => {
    ...
    expect((await ethers.provider.getBlock("latest")).number).to.equal(14083004);
    ...
    ac019= await ethers.getSigner("0x8Ada5F216eBA7612682b64C9fd65D460bFed264F");
    ...
    await axolittlesContract.connect(n8).setApprovalForAll(stakingContract.address,1);
    await axolittlesContract.connect(ac019).setApprovalForAll(stakingContract.address,1);
    //console.log("Contract deployed to:", stakingContract.address);
  });
  //AUX CONTRACT CHECKS:
  it("should fork existing contract state at block 14083004", async () => {
    expect(await axolittlesContract.owner()).to.equal("0xb0151D256ee16d847F080691C3529F316b2D54b3");
    expect(await bubblesContract.owner()).to.equal("0xb0151D256ee16d847F080691C3529F316b2D54b3");
  ...
  it("should fail when staking mixed owned then not owned together", async () => {
    ...
    expect(await axolittlesContract.balanceOf(n8.address)).to.equal(103);
    expect(await axolittlesContract.balanceOf(stakingContract.address)).to.equal(0);
  });
  ...
  */
  ...
  it("should pass with legitimate stake", async () => {
    await expect(
      stakingContract.connect(n8).stake([4504,7027,5803])
    ).to.emit(stakingContract,'Stake').withArgs(n8.address, [4504,7027,5803]);
    expect(await axolittlesContract.balanceOf(n8.address)).to.equal(100);
    expect(await axolittlesContract.balanceOf(stakingContract.address)).to.equal(3);
  });
  ...
  it("should fail when unstaking mixed owned (staked) then not owned together", async () => {
    await expect(
      stakingContract.connect(ac019).stake([8052])
    ).to.emit(stakingContract,'Stake').withArgs(ac019.address, [8052]);
    await expect(
      stakingContract.connect(n8).stake([4504,7027,5803])
    ).to.emit(stakingContract,'Stake').withArgs(n8.address, [4504,7027,5803]);
    await expect(
    ...
    expect(await axolittlesContract.balanceOf(n8.address)).to.equal(100);
    expect(await axolittlesContract.balanceOf(stakingContract.address)).to.equal(4);
  });
      //b. not owned first, then owned    
  ...
  */
  ...
  it("should pass with legitimate unstake with delay", async () => {
    ...
    await expect(
      stakingContract.connect(n8).unstake([4504,7027])
    ).to.emit(stakingContract,'Unstake').withArgs(n8.address, [4504,7027]);
    expect(await axolittlesContract.balanceOf(n8.address)).to.equal(102);
    expect(await axolittlesContract.balanceOf(stakingContract.address)).to.equal(1);
  });
  ...
  it("should fail claim when no axos staked, no bubbles generated", async () => {
    await expect(
      stakingContract.connect(n8).claim()
  ...
  it("should allow claim with bubbles remaining but axos unstaked", async () => {
    ...
    //unstake
    await expect(
      stakingContract.connect(n8).unstake([4504,7027,5803])
    ).to.emit(stakingContract,'Unstake').withArgs(n8.address, [4504,7027,5803]);
    //claim
    await expect(
      stakingContract.connect(n8).claim()
    ).to.emit(stakingContract, 'Claim').withArgs(n8.address, '90000000000000000');
  });
  ...
  it("should check reward when bubbles remain, axos unstaked", async () => {
    ...
    expect(await stakingContract.connect(n8).checkReward(n8.address)).to.equal('90000000000000000');
    expect(await stakingContract.connect(ac019).checkReward(ac019.address)).to.equal(0);
  });
  ...
  it("should check reward when bubbles remain, axos staked", async () => {
    ...
    await hre.ethers.provider.send('evm_increaseTime', [7 * 24 * 60 * 60]);
    await network.provider.send("evm_mine");
    expect(await stakingContract.connect(n8).checkReward(n8.address)).to.equal('45000000000000000');
    expect(await stakingContract.connect(ac019).checkReward(ac019.address)).to.equal(0);
  });
